...
tags:
  - Workflow
lede: "Port Tanuj's canonical-archive intent first, then force-push-with-lease — and skip his `any` types, which would undo the type audit."
summary: "Working note that archives a collaborator's branch verbatim before overwriting it: full source of his citations.css, the modified CitationModal.ts, the citationFileService additions, and the settings auto-save gating. The short section at the top is the five-item port checklist plus the force-push-with-lease strategy and three open questions; everything below is archived source kept only as the porting reference. Use it when reimplementing that functionality on the refactored, type-safe codebase."
publish: false
---
